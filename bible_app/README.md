# Flutter Bible — разработка

Исходный код приложения «Библия». Краткий обзор репозитория и структура папок — в **[README в корне репозитория](../README.md)**.

## 🔔 Последний релиз

- `1.1.1+33` (2026-05-19): исправлена вёрстка диалога выбора главы на реальных телефонах; доработки push, полоски копирования и пикеров книги/главы (см. [CHANGELOG.md](CHANGELOG.md)).
...
1. Подготовить юридическую и учетную часть
Завести/проверить аккаунт разработчика в RuStore (физлицо/ИП/юрлицо).
Подготовить данные для карточки разработчика: название, контакты, сайт/почта поддержки, политика конфиденциальности.
Убедиться, что у вас есть право на весь контент приложения (тексты, иконки, шрифты, изображения).
2. Подготовить релизный Android-артефакт
Для RuStore обычно загружают APK (иногда поддерживается и AAB, но для вашего текущего процесса APK проще и стабильнее).
Собрать релизную сборку: flutter build apk --release.
Проверить подпись релизным ключом и сохранить keystore в безопасном месте.
Увеличить versionCode/версию (у вас это уже встроено через pubspec.yaml + релизные манифесты).
3. Прогнать предрелизную проверку качества
Проверить старт приложения, навигацию по вкладкам, поиск, план чтения, блокнот.
Проверить обновление поверх предыдущей версии (install over install).
Проверить Android 8–14+ (минимум 2–3 устройства/эмулятора).
Проверить критичные разрешения и что нет «битых» ссылок/пустых экранов.
4. Подготовить Store-материалы
Иконка приложения (обычно 512x512), скриншоты (телефон, при необходимости планшет), баннер/обложка по требованиям RuStore.
Короткое и полное описание приложения на русском.
Категория, теги, возрастной рейтинг.
Текст «Что нового» для текущего релиза.
5. Оформить обязательные документы и декларации
Политика конфиденциальности (публичный URL).
Описание обработки данных: что собирается, зачем, где хранится.
Если есть аналитика/краш-репорты/внешние API — отразить это в форме данных.
Проверить соответствие законам РФ (персональные данные, пользовательское соглашение при необходимости).
6. Создать карточку приложения в RuStore
Заполнить метаданные: название, описание, категория, контакты.
Загрузить графику и скриншоты.
Загрузить APK и убедиться, что версия/подпись корректно распознаны.
Добавить release notes (на основе вашего CHANGELOG.md).
7. Пройти модерацию
Отправить сборку на проверку.
Оперативно отвечать на замечания модерации (обычно про описание, контент, политику, тех.ошибки).
При отклонении: исправить, увеличить версию, загрузить новый APK и повторно отправить.
8. После публикации: релизный цикл
Проверить карточку в магазине и установку «с нуля».
Отслеживать отзывы/краши/ANR, быстро выпускать hotfix.
Для каждого релиза повторять ваш текущий pipeline:
код → тест → version bump → CHANGELOG/manifest → сборка APK → загрузка в RuStore.

## RuStore и Google Play — подготовка
...
**Важно:** APK с прежним `com.example.bible_app` не обновится поверх нового package — для пользователей Drive это отдельная установка.

---

Этап 1. Аккаунт разработчика RuStore
1.1. Регистрация
Откройте RuStore Консоль.
Войдите через VK ID (телефон, почта, Яндекс или Госуслуги).
Выберите тип аккаунта:
Физлицо — проще стартовать; нужна верификация (паспорт + проверка с камерой). Подходит для бесплатного приложения без сложной монетизации.
ИП / юрлицо — для коммерции, выплат, B2B; часто нужна КЭП и подпись регистрационной формы; доступ обычно 2–4 рабочих дня.
Документация: Регистрация разработчиков.

1.2. Профиль разработчика
Подготовьте заранее:

Поле	Что указать
Название разработчика
Как вас должны видеть пользователи
Email поддержки
Рабочая почта, отвечаете в течение нескольких дней
Сайт
По желанию; можно страницу на GitHub Pages / лендинг
Политика конфиденциальности
Публичный URL (обязательно для модерации)
Этап 2. Юридика и контент (для Библии это критично)
2.1. Права на тексты
В приложении — Синодальный перевод и элементы Септуагинты. Для карточки в магазине нужно быть готовым объяснить:

что тексты не нарушают авторские права;
что вы не продаёте «пиратскую» Библию.
Если есть лицензия/разрешение или текст в общественном достоянии — кратко сформулируйте это для себя (модератор может спросить).

2.2. Политика конфиденциальности
Разместите HTML-страницу (GitHub Pages, сайт, Google Sites) с описанием:

какие данные собираются (у вас в основном локально: заметки, закладки, прогресс плана);
INTERNET — для проверки обновлений (release-manifest с Google Drive);
нет ли аналитики, рекламы, аккаунтов;
контакт для запросов по персональным данным.
Шаблон: что хранится на устройстве, что уходит в сеть, срок хранения, email разработчика.

2.3. Пользовательское соглашение
Для бесплатного офлайн-приложения часто достаточно политики конфиденциальности; при монетизации или подписках — отдельное соглашение.

Этап 3. Техническая подготовка APK
3.1. Уникальный package name
Выберите финальный ID, например: ru.februaryidea.bible.
Поменяйте в android/app/build.gradle (applicationId и namespace).
При необходимости — пакеты Kotlin/Java и AndroidManifest.
Не публикуйте в RuStore, пока не уверены в ID — потом не изменить.
3.2. Подпись release
Создайте keystore (как в вашем README):
keytool -genkey -v -keystore c:\Key\Bible\upload-keystore.jks -storetype JKS -keyalg RSA -keysize 2048 -validity 10000 -alias upload
Файл android/key.properties (не коммитить в git):
storePassword=***
keyPassword=***
keyAlias=upload
storeFile=../../upload-keystore.jks
В build.gradle для release укажите signingConfig signingConfigs.release.
Сохраните keystore и пароли — без них нельзя выпускать обновления в RuStore.
Документация RuStore: Подпись APK.

3.3. Версия сборки
В pubspec.yaml:

version: 1.1.1+23   # +23 = versionCode для Android
Для каждого обновления в RuStore versionCode должен расти, подпись — та же, package — тот же.
...
Проверьте на 2–3 устройствах: установка, обновление поверх старой версии, Библия / поиск / план / блокнот / настройки.

3.5. Формат: APK или AAB
APK — проще, совпадает с вашим текущим процессом (Drive + RuStore).
AAB — RuStore поддерживает; подписи нужно загружать отдельно. Для Google Play позже почти наверняка понадобится AAB.
Этап 4. Материалы для карточки в магазине
4.1. Тексты
Поле	Требования / совет
Название в консоли
При создании приложения
Название в каталоге
До 30 символов, уникальное
Краткое описание
1–2 предложения, суть
Полное описание
Функции: чтение, поиск, планы, блокнот, темы, обновления
«Что нового»
Из CHANGELOG.md текущего релиза
Категория
Например «Книги», «Образование» или ближайшая по смыслу
4.2. Графика
Ассет	Требования RuStore
Иконка
512×512, PNG/JPG, до 3 МБ, без прозрачных краёв, фон на всю площадь
Скриншоты телефона
Обязательно, 1–10 шт., PNG/JPG, до 3 МБ, ориентация вертикальная или горизонтальная, ~16:9
Планшет
Опционально (от 7")
Видео
Опционально, ссылка VK Видео
Рекомендуемые скриншоты для вашего приложения:

Глава Библии со стихами.
Поиск с результатами.
Избранное.
План чтения.
Блокнот / настройки (светлая и тёмная тема).
4.3. Контакты в карточке
Минимум одно из: email, сайт, группа ВК.
Email поддержки лучше указать обязательно.

Этап 5. Создание приложения в консоли
Добавить приложение может только «Владелец компании». Загружать сборки могут также Администратор, Релиз-менеджер, Разработчик.

5.1. Новое приложение
Консоль → Приложения → Добавить приложение.
Введите рабочее название → Добавить.
Тип: Универсальное (телефон/планшет) — не ТВ, если приложение только для смартфонов.
5.2. Загрузка версии
Откройте приложение → Загрузить версию.
Загрузите APK (или AAB).
Дождитесь этапов:
Загрузка — не закрывайте страницу.
Обработка — можно уйти; черновик хранится 14 дней, потом файл удалят.
Проверьте, что консоль увидела:
package name;
versionCode / versionName;
подпись.
Требования к APK: документация — размер до 5 ГБ, уникальный package, подпись, для обновлений — тот же package и та же подпись, versionCode больше предыдущего.

5.3. Безопасность данных и разрешения
Заполните блок «Безопасность данных пользователя»:

INTERNET — обоснование: проверка обновлений, загрузка manifest/APK (если используете).
Локальное хранение — заметки, закладки, настройки (на устройстве).
Если нет рекламы и аналитики — явно укажите «не собираем» / «только локально».
Документация: Декларирование разрешений.

5.4. Остальные поля версии
Что нового — текст релиза.
Категория, возрастной контент (для книг/религии — без насилия, без запрещённого контента).
Контакты разработчика.
Иконка, скриншоты.
Минимальная версия Android — подтянется из манифеста (у вас minSdk 23+).
5.5. Отправка на модерацию
Проверьте черновик целиком.
Отправить на модерацию.
Среднее время — около 1 часа, иногда дольше.
При отклонении — письмо/комментарий в консоли; исправить → увеличить versionCode → новый APK → снова на модерацию.
Типичные причины отклонения: нет политики конфиденциальности, падение при старте, несоответствие скриншотов реальному UI, сомнения по контенту/авторским правам.

Этап 6. После публикации
Установите приложение из RuStore на чистое устройство (не только APK с ПК).
Проверьте обновление: старая версия → новая из консоли (internal/тест, если есть).
Следите за отзывами в карточке.
Для каждого релиза повторяйте ваш pipeline:
код → тест → version в pubspec.yaml → CHANGELOG + manifest → flutter build apk --release → загрузка в консоль.
APK на Google Drive можно оставить для текущих пользователей, но для RuStore лучше указать в описании, что актуальные обновления — через магазин (или синхронизировать versionCode и подпись, если хотите один канал обновлений).

Чек-лист перед первой отправкой (кратко)
#	Задача	Статус у вас
1
Финальный applicationId (не com.example.*)
Нужно сделать
2
Release keystore + key.properties
Проверить README / gradle
3
flutter build apk --release с релизной подписью
4
...
Описание + «Что нового»
9
Декларация разрешений (INTERNET)
10
Аккаунт RuStore (физлицо/ИП)
11
Тест на Android 8–14+
Подготовка к Google Play (кратко, следующий шаг)
RuStore	Google Play
APK удобен
Обычно нужен AAB: flutter build appbundle --release
Своя консоль
Google Play Console (~$25 разово)
Политика конфиденциальности
Обязательно + Data safety
Package + подпись
Тот же applicationId; Play App Signing
Имеет смысл сначала зафиксировать package name и keystore под RuStore — те же значения пойдут в Google Play.

Если нужно, следующим сообщением могу: (1) предложить конкретный applicationId и правки в build.gradle, (2) черновик политики конфиденциальности под ваше приложение, (3) тексты для карточки RuStore (название, описание, «Что нового» для 1.1.1+23).
